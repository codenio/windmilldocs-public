...
- TPC-H provides a set of canonical queries. They perform numerous joins, aggregations, group-bys, etc. 9 of them (spec Q1, Q3, Q5, Q6, Q10, Q12, Q14, Q16, Q18) were converted in the different dialects.
...
:::info Updated July 2026

The charts below have been **refreshed on current engine versions** (Polars 1.42, DuckDB 1.5, Spark 4.0 with AQE, JDK 17). A few honest corrections to the original 2023 run:

- **This refresh is not comparable to the 2023 numbers.** It was re-run on a single local **AMD Ryzen 9 9900X** (12 cores / 24 threads, 60 GB RAM), reading Parquet from S3 (MinIO on localhost) — not AWS. Read the *relative* engine story, not the absolute seconds. The scale factors are now **1G / 10G / 50G / 100G** (the larger-than-memory regime only shows up past ~25G on a 60 GB box).
- It is a **TPC-H-derived, unaudited** benchmark of **9 queries** (the original text's "8" was a miscount), and it benchmarks the *libraries* on one worker — Windmill supplies orchestration and S3 glue, not a compute speed-up.
- **DuckDB now queries the Parquet directly** (projection/filter pushdown) instead of materializing every table first, which is ~10× faster at scale and the idiomatic way to write a DuckDB step. **Polars' new streaming engine** replaces the old, slow "lazy" mode.
- **Polars "eager" still runs out of memory** once the working set exceeds RAM (now at 50G+, shown as 0) — that 2023 finding holds, it just scales with the box. We also fixed three real bugs in the 2023 query translations so all engines now return identical results.

Full method, pinned versions, raw per-run data and a critical write-up: [windmill-labs/windmill-benchmarks](https://github.com/windmill-labs/windmill-benchmarks/tree/main/pipelines).

:::

<BarChart
	title="Duration of the 9 queries ran sequentially (in seconds)"
	yTitle="duration (in seconds)"
	labels={['Bench 1G', 'Bench 10G', 'Bench 50G', 'Bench 100G']}
	rawData={[
		{
			label: 'Spark (single node)',
			data: [9.0, 24.0, 77.9, 149.1]
		},
		{
			label: 'Windmill + DuckDB',
			data: [0.3, 2.0, 9.8, 20.3]
		},
		{
			label: 'Windmill + Polars (streaming)',
			data: [0.5, 4.3, 27.3, 55.4]
		},
		{
			label: 'Windmill + Polars (eager)',
			data: [0.5, 4.6, 0, 0]
		}
	]}
/>
...
<BarChart
	title="Memory peak for the run of the 9 queries ran sequentially (in GB)"
	yTitle="Memory peak (in GB)"
	labels={['Bench 1G', 'Bench 10G', 'Bench 50G', 'Bench 100G']}
	rawData={[
		{
			label: 'Spark (single node)',
			data: [7.1, 21.5, 24.6, 29.3]
		},
		{
			label: 'Windmill + DuckDB',
			data: [0.5, 2.4, 10.2, 19.8]
		},
		{
			label: 'Windmill + Polars (streaming)',
			data: [0.9, 4.1, 19.0, 34.2]
		},
		{
			label: 'Windmill + Polars (eager)',
			data: [2.8, 21.8, 0, 0]
...
On the refreshed run, **DuckDB (querying the Parquet directly) is fastest at every scale** and the most memory-efficient engine that still scales — the full 9-query workload over the 100G dataset runs in about **20 seconds using ~20 GB of RAM**. **Polars' streaming engine** finishes it in ~55 seconds at ~34 GB. Both beat the single-node Spark baseline (~150 seconds), which — even tuned with AQE — carries a large fixed overhead that makes it 20–50× slower at the smaller scales. Polars' **eager** mode is fast up to 10G but runs out of memory beyond that (shown as 0); this is where you reach for streaming.
The 100G dataset contains a table with up to 600 million rows, on a single 60 GB desktop. So yes, this doesn't scale horizontally — but it confirms that the large majority of data pipelines can be handled by one large-enough node. And when you think about it, what's more convenient? Managing a single beefy server or a fleet of small servers?

:::info Larger than memory

DuckDB and Polars both spill to disk when the working set exceeds RAM, so you can go well past what fits in memory. In the refresh, a memory-capped DuckDB configuration processed the full 100G workload at a **13 GB** peak by spilling (trading some speed for a tiny footprint), and Polars' streaming engine handled it out-of-core. This pairs naturally with Windmill flows that share a directory between steps. (The original 2023 post reported an on-disk DuckDB flow completing the 100G benchmark in 40 minutes at 29.1 GB; current versions are dramatically faster.)
