...
authors: [hugocasa]
tags: ['ai', 'windmill', 'support', 'claude']
description: 'How does Windmill automate its own support? A Windmill pipeline funnels Discord, Slack and email tickets into one Discord queue, triages them with Claude, drafts replies, and dispatches fixes.'
title: 'Automating quality support at scale: AI and human in the loop'
image: ./thumbnail.png
---

import DocCard from '@site/src/components/DocCard';
import HubProjectEmbed from '@site/src/components/HubProjectEmbed';
import walkthroughVideo from './support-automation-walkthrough.mp4';
...
>
    <source src={walkthroughVideo} />
    <a href={walkthroughVideo}>Download the video</a>.
</video>
...
Each channel has its own way in, and they all feed the same entry [flow](/docs/flows/flow_editor):

- **Slack** Connect channels hit a [webhook](/docs/core_concepts/webhooks). The flow auto-joins new ones, so we never miss a customer's first message.
- **Email** to a dedicated address comes in through Windmill's [email trigger](/docs/triggers/email_triggers), normalized into the same shape as everything else.
- **Discord** messages come in over a Windmill [WebSocket trigger](/docs/triggers/websocket_triggers), so new messages in our channels and threads land in real time.
...
```mermaid
flowchart TD
  S[Slack] --> ENTRY[Entry flow]
  E[Email] --> ENTRY
  D[Discord] --> ENTRY
  G[GitHub issues] -->|mirrored to Linear| ENTRY
  ENTRY -->|fires async| PP[Triage post-processing]
  PP --> AN[Claude analysis in a sandbox]
  AN --> DR[Auto-generated draft reply]
  DR --> A[Human approves]
  A --> OUT[Reply sent to the channel the customer wrote from]
  AN --> F[Auto-dispatch clear-cut fixes]
  F --> WM[webmux drafts a PR]
  WM --> PRN[PR link in the thread, customer pinged on merge]
```
...
![A support request landing in our Discord queue, with the customer dossier the bot attaches before it starts analyzing](./request-and-dossier.webp)

Whatever the channel, the entry flow does the same housekeeping: it drops noise (a thank-you or some off-topic banter never gets past a cheap classification step), stages attachments to [object storage](/docs/core_concepts/object_storage_in_windmill), and handles team commands like assign and close.
...
The entry flow doesn't run the heavy analysis inline: a step enqueues the post-processing flow as a separate job, one `runFlowByPath` call with the [TypeScript client](/docs/advanced/clients/ts_client), and returns.
...
![The entry flow: it routes each message by source, filters noise, matches it to a thread, and fires the heavy analysis asynchronously](./entry-flow.webp)

Several of those steps, the noise filter, the thread matcher, the owner routing, are Windmill [AI agent steps](/docs/core_concepts/ai_agents), the same building block any flow can drop in to let a model make a small decision with tools.

## Triage with Claude

![The async post-processing flow in the Windmill editor: analyze the ticket, publish the triage and draft, then a guarded dispatch](./post-processing-flow.webp)
...
The async step runs Claude in a [sandboxed](/docs/core_concepts/ai_sandbox) Windmill [script](/docs/getting_started/scripts_quickstart), with all the context it needs:
...
![The triage the AI posts back into the thread: root cause with file-and-line references, a check for existing issues, and the customer's version](./ai-triage.webp)
...
So every customer-facing reply goes through an [approval step](/docs/flows/flow_approval) first.
...
![The drafted reply in the thread, with the controls to send it as-is or edit it first](./draft-and-approval.webp)
...
![What the customer sees: the reply, signed by the human who approved it and marked as AI-drafted](./sent-reply.webp)

```mermaid
flowchart TD
  AN[Claude analysis] -->|if a draft exists| A[Branch A: auto-reply]
  AN -->|if it found a clear fix| B[Branch B: dispatch a fix]
  A --> A1[Post draft: send as-is or edit]
  A1 --> A2[Wait up to 24h]
  A2 -->|approved| A3[Send signed reply]
  B --> B1[Describe the bug, not the customer]
  B1 --> B2[Pick an owner, open a Linear issue]
  B2 --> B3[webmux drafts a PR]
  B3 --> B4[PR link back in thread, customer notified on merge]
```
...
From there [webmux](/blog/webmux), our open-source dashboard for parallel AI coding agents, takes over.
...
![A teammate tells it to dispatch from the thread; the AI re-analyzes, drafts the fix, and hands it to webmux](./dispatch-to-webmux.webp)
...
![The team thread as the fix lands: the PR link when webmux drafts it, the merge notice a few hours later, then the release](./merged-and-released-in-thread.webp)

![The customer's Slack, pinged automatically as their fix merges and then ships in a release](./customer-merge-and-release-notifications.webp)
...
One recent example: a UI bug where dialogs rendered behind the new AI chat panel, [fixed here](https://github.com/windmill-labs/windmill/pull/9636).
...
The customer hears their issue shipped even though nobody manually connected the dots.

## Nothing slips through

A [scheduled](/docs/core_concepts/scheduling) job runs every 15 minutes and pings the relevant team lead about any thread that's gone more than two hours without a reply.
And every action the pipeline takes is recorded, so re-runs never double-post and a fix is never dispatched twice for the same thread.

## Where it stops, and where it doesn't fit

It's good at the technical tickets and noticeably weaker at the rest.
A bug report with a stack trace, a "why is my flow doing this", a config question: those have answers in the code and the docs, which is exactly what the AI is holding.
Commercial questions are different.
They need context the system doesn't have and decisions that are ours to make, an MSA to negotiate, a pricing exception, a roadmap promise, so those reach a human early, on purpose.

We also didn't set out to build a help desk.
We meet customers in their own Discord and Slack channels and only pull a conversation aside when it really needs it.
The tracking a help desk would give us is the easy part, and we get it for free by funneling every channel into one place.
The hard part, and the whole reason this exists, is the layer on top: triage, drafted replies, and fixes that land as real pull requests against our own code.
That last part is the one a help desk structurally can't do, because it lives outside your repo.

## How we built it: locally, with Claude

A quick word on how all this got built.
None of it was clicked together in a UI.
It was written locally with [Claude Code](https://docs.anthropic.com/en/docs/claude-code), against the workspace files on disk.

That works because everything in Windmill, scripts, flows, triggers, schedules, can live as plain files in a git repo.
The workspace is [git-synced](/docs/advanced/git_sync): push to `main`, CI runs `wmill sync push`, and it's in production.
(Git sync is a Cloud and [Enterprise Edition](/pricing) feature, with Community Edition covering workspaces of up to 2 users.)
And `wmill init` generates an `AGENTS.md` plus skill files that tell an agent exactly how those files work, so Claude builds valid scripts, flows and triggers without hand-holding.
The loop ends up looking like normal engineering: Claude edits files, we review the diff like any other PR, a merge ships it.

```mermaid
flowchart LR
  C[Claude Code, local] -->|edits scripts and flows| WS[Workspace files in git]
  WS -->|push to main| CI[CI runs wmill sync push]
  CI -->|deploy| PROD[Production workspace]
  PROD -->|git sync| WS
```

And none of this is a separate product.
It's scripts, flows, triggers, and a handful of tables on the same Windmill that runs the rest of what we do.
That's the real takeaway for us: because it's all files and building blocks an AI already understands, Windmill turned out to be very easy to build on this way, and the pipeline keeps growing the same way it started.

<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		title="Local development with AI"
		description="Generate AI context files with wmill init and build scripts and flows with Claude Code or Cursor."
		href="/docs/misc/guides/local_dev_with_ai"
	/>
	<DocCard
		title="Git sync"
		description="Keep a Windmill workspace in sync with a git repository and deploy on push."
...
Every step has a demo mode: with no credentials filled in, it logs what it would send and returns canned data, so you can import it and run the whole loop, approval page included, before wiring up Slack, Anthropic or GitHub.

<HubProjectEmbed
	slug="support-automation"
	focus="flow-86"
	title="The triage post-processing flow of the support automation project on the Windmill Hub"
	height={924}
	wide
/>
...
Building locally with Claude works, but it used to mean leaving Windmill to do it.
That gap has since closed: [AI sessions](/docs/core_concepts/ai_sessions) (beta) bring the same coding-agent power right into the browser.
You open a session, chat with Claude on one side and watch a live editor and preview on the other, ask it to build or change a script, flow or app, review the diff, and deploy when you're happy.
It's the local-dev workflow minus the local part, scoped to a forked workspace so it never steps on what's already running.

The way we work on this pipeline shaped that product directly: supervising coding agents on our own codebase through webmux is what taught us [what AI sessions needed to be](/blog/how-webmux-shaped-ai-sessions).
And it's exactly the kind of thing the pipeline above will lean on to get even more hands-off.
