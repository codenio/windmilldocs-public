...
AI agent workflows can be [triggered](/docs/triggers) through multiple mechanisms. Use [webhooks](/docs/core_concepts/webhooks) or [HTTP endpoints](/docs/triggers/http_routing) to invoke agents programmatically from external systems. These triggers support both streaming and non-streaming modes, allowing you to choose whether to receive the agent's response incrementally or wait for the complete result.
