...
Windmill supports SSO via [Azure AD OAuth](/docs/advanced/setup_oauth). The same identity can authenticate against MSSQL using `aad_token`, so one login covers both platform access and database access.
...
Enterprise Kafka clusters in AD environments often use Kerberos. Windmill's [Kafka triggers](/docs/triggers/kafka_triggers) support `SASL_GSSAPI` and `SASL_SSL_GSSAPI` with keytab support (file path or base64-encoded):
