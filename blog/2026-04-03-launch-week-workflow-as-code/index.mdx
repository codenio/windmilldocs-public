...
Other workflow-as-code frameworks leave the execution somewhere you have to run yourself: Temporal polls its task queues from worker processes you deploy, Inngest invokes an HTTP handler inside your own application. We wanted the same capabilities with two annotations and zero new infrastructure.
...
**Zero worker waste.** When a workflow suspends (sleep, approval, waiting for child jobs), the worker is fully released and keeps nothing in memory. The next round rebuilds the workflow from its checkpoint on any available worker, so you pay only for compute you actually use.
