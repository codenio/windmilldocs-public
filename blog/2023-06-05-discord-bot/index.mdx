...
<sup>
	<i>
		...
		documentations, surrounded by books --aspect 3:2
	</i>
</sup>
...
Each iteration of the for-loop are individual jobs and can be run [in parallel](/docs/flows/flow_branches#branch-all), [retried](/docs/flows/retries) and [error handled](/docs/flows/flow_error_handler) independently.
...
Directly from Windmill, we picked the script that fetches the content of a repository from GitHub and forked it.
...
Discord mandates that we verify the request before responding to it, so we need to include the `X-Signature-Ed25519` and `X-Signature-Timestamp` headers in the request to verify the Discord request. We also added the `raw` query parameter to get the raw stringified body.
