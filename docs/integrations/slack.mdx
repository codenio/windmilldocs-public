...
The Slack integration is done through OAuth. On [self-hosted instances](../advanced/1_self_host/index.mdx), integrating an OAuth API will require [Setup OAuth and SSO](../advanced/27_setup_oauth/index.mdx).
...
One simpler way to handle permissions is to host resources and variables on a [folder](../core_concepts/8_groups_and_folders/index.mdx#folders) that is part of the [group](../core_concepts/8_groups_and_folders/index.mdx#groups) `slack`.
...
acting on Slack through a connected Slack app.
...
### Which identity messages are published under

Messages sent using the resource created above are published by the Slack app the OAuth connection went through, not under your own name, since the flow grants that app a bot token (`xoxb-`).
On Windmill cloud that app is the Windmill Slack app; on a self-hosted instance it is the app your administrator registered in the instance OAuth settings.
The `chat:write.public` scope it requests lets it post to any public channel without being invited to it.

The identity follows the token, so a resource holding a user token (`xoxp-`) still posts under that user's name: one you filled in by hand, or one issued by an OAuth connection made before Windmill moved to Slack's v2 flow.
Slack tokens do not expire unless the app turns on [token rotation](https://docs.slack.dev/authentication/using-token-rotation/), so those resources keep working as they are; reconnect them to get a bot token.

The Slack resource created in the [Action on Windmill from Slack](#action-on-windmill-from-slack) section publishes messages the same way, through the same instance-level app unless the workspace [configures its own](#workspace-level-slack-app-configuration).
It also pairs the Slack workspace with the Windmill workspace, which is what enables slash commands, mention triggers and approvals.
