...
On [self-hosted instances](../advanced/1_self_host/index.mdx), integrating OAuth APIs will require [Setup OAuth and SSO](../advanced/27_setup_oauth/index.mdx). Providers that authenticate server-to-server can be connected with the [client credentials](../advanced/27_setup_oauth/index.mdx#client-credentials) grant, with either instance-wide or per-resource client credentials.
...
| [Google Workspace (gworkspace)](./gworkspace.md) | Manage Google Workspace users, groups, org units and security via the Admin Directory API |
...
| [FaunaDB](./faunadb.md)                           | Deprecated - the Fauna cloud service was shut down in May 2025                                                             |
...
| [Tigris](./tigris.mdx)                            | S3-compatible object storage service with edge caching and zero egress fees                                               |
