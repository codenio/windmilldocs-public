...
We provide [priority support](../3_support_and_sla/index.mdx) to our [Enterprise](/pricing) customers. This support includes the implementation of feature requests, prioritized bug resolutions, and recommendations on use cases.
...
Any organization or individual is welcome to apply to Windmill's partnership program.
...
To apply, send an email to [partners@windmill.dev](mailto:partners@windmill.dev) with material that shows what you have already built.
We review applications from this material first, without a call, so the more concrete your application is, the faster we can evaluate it.
This applies in particular to individuals and freelancers.

Your application should include:
- The [skills](#skills) you are applying for.
- A short video (a few minutes is enough) where you walk us through Windmill projects you have built, ideally with a screen share of the actual scripts, flows, apps or infrastructure.
- A description of each project: the problem it solved, who used it, its scale (users, executions, workers) and your role in it.
- Any public work you can point to, such as Git repositories, [Hub](https://hub.windmill.dev/) contributions, blog posts or references from customers.
- Your company name if any, location, languages and the time zones you can work in.

If the material is not enough to evaluate a skill, we may ask for more examples or schedule a call.
...
The test starts with the material you send when you [apply](#how-to-apply): for each skill, show an example of your work that covers it (e.g., a recorded walkthrough of a complex app if you are applying to [Build and maintain complex apps](#build-and-maintain-complex-apps)).
A member of the Windmill core team reviews it and, when needed, follows up with questions or a call.
...
- [Sleep / Delays in flows](../../flows/15_sleep.md)
...
Be able to set up a custom [local development](../../advanced/4_local_development/index.mdx) environment, including a [git workflow](../../advanced/9_deploy_gh_gl/index.mdx) between a staging and a production workspace with [version control](../../advanced/12_deploy_to_prod/index.mdx#version-control).
...
You know how to configure [SAML, SCIM](../../advanced/28_saml_and_scim/index.mdx) and other [authentication methods](../../core_concepts/15_authentication/index.mdx) for large organizations.
