...
Non-profits & universities benefit from the regular Enterprise plan at a 40% discount.
...
- To upgrade to the Team edition, open the <a href="https://app.windmill.dev/workspace_settings?tab=premium" rel="nofollow">Premium plans</a> tab of your workspace settings on Windmill cloud.
...
- Once you contact us, we will set up your subscription and you will get access to a dedicated Windmill instance on the cloud.
...
- To have a fully Enterprise Edition Windmill, we recommend also setting up:
	- [SSO/SAML](../../advanced/27_setup_oauth/index.mdx) for user management and authentication.
...
The subscription to the Enterprise Self-Host offer starts from the [trial portal](https://portal.windmill.dev/trial), with a 1-month free trial. You can reach out to us via sales@windmill.dev, [Discord](https://discord.com/invite/V7PM2YHsPB), or schedule a [meeting](https://www.windmill.dev/book-demo) with the founder for an introduction session.
...
For new enterprise customers, we recommend following the [Enterprise onboarding guide](../2_onboarding/index.mdx) to get started.
...
Calculation: 3 workers × 1 CU each + 4 small workers × 0.5 CU each + 2 report workers × 1 CU each + 1 native worker with 8 native subworkers = 8 CUs
...
To deploy the self-hosted package on AWS (EKS cluster, RDS database, then install via the [Helm chart](https://github.com/windmill-labs/windmill-helm-charts)), follow [Windmill on AWS EKS or ECS](../../advanced/1_self_host/aws_eks_ecs.mdx).
...
The portal hosts the feature request and issue dashboard for [Enterprise](/pricing) customers, and is where its settings (authorized domains, linked GitHub users, email notification addresses) are configured. See [Feature request and issue dashboard](../3_support_and_sla/index.mdx#feature-request-and-issue-dashboard) for details and usage.
