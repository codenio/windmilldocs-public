...
The bottom panel is the [Runnable panel](./3_app-runnable-panel.mdx).
...
The right panel is the [Component library](./4_app_configuration_settings/1_app_component_library.mdx) and [Component settings](./4_app_configuration_settings/3_app_component_configuration.mdx) panel.
...
In the menu, you can view the [deployment](#deploy) history of the app. It displays the date and time of the deployment, the user who deployed the app, and the version of the app.
...
![Deployment history](../assets/apps/1_app_toolbar/deployement-history.png.webp 'Deployment history')
...
<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		...
		href="/docs/apps/app_debugging#troubleshoot-panel"
		color="orange"
	/>
</div>
...
Apps are automatically [saved as drafts](../core_concepts/0_draft_and_deploy/index.mdx#draft) as you edit. The autosave indicator in the toolbar shows the current save status. The first time you save the app, you'll be asked to select the owner of the app. You can choose either a user or a [folder](../core_concepts/8_groups_and_folders/index.mdx#folders).

## Deploy

Windmill apps are custom-made user interfaces meant to interact with scripts and flows.

The Access control in the deploy menu decides who can open a deployed app:

- **Members**: The app can be accessed & triggered by anyone who has [read access](../core_concepts/16_roles_and_permissions/index.mdx) to the app. This is the default.
- **Guests**: The above, plus anyone with no Windmill account whom your identity provider authenticates or a [guest JWT](./13_guest_apps/index.mdx#guest-jwt-for-embedded-apps) names. See [guest apps](./13_guest_apps/index.mdx).
- **Public**: The app can be accessed as a standalone app by anyone who has the secret URL, with no login. See [public apps](./8_public_apps.mdx).

The same panel also holds [sandbox isolation](../full_code_apps/8_sandbox_isolation/index.mdx) (alpha), which confines the app's browser-side code so it cannot reach the viewer's Windmill session.
