...
> [Example repo](https://github.com/windmill-labs/windmill-sync-example) opened in VS Code. We see 2 flows and 1 script, the flows are their own folders, each step in a flow is a separate file in their respective language. Scripts have their metadata in a separate file.
...
The preview & run will work for any script meeting the specific language requirements (main function, imports) and being named with the dedicated file extension (.py, .go etc.). For scripts in Deno, name the file \[name\].deno.ts, ".ts" being by default Bun (configurable with the `defaultTs` field in your [wmill.yaml](../../advanced/3_cli/sync.mdx#wmillyaml)).
