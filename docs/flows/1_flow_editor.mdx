...
- Click the minimize button on the group to collapse it into a single node (similar to a [subflow](#subflows)). Click again to expand.
...
A step can run another flow of the workspace instead of a script: pick `Flow` when inserting a step. Hover a subflow node and click "Expand subflow" to inline its steps in the graph, "Unexpand subflow" on the expansion's boundary to collapse it back.

Selecting an inlined step shows a read-only view of it, a breadcrumb of the subflow chain it belongs to, an "Edit" button that opens the owning subflow in a drawer with that step selected, and a button to open it in a new tab.
...
<div className="grid grid-cols-2 gap-6 mb-4">
	...
	<DocCard
		...
		description="There are multiple ways to handle errors in Windmill flows."
