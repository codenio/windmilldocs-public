...
To open a single app to people who have no Windmill account, without granting them anything else in the workspace, use a [guest JWT](../../apps/13_guest_apps/index.mdx#guest-jwt-for-embedded-apps) instead.
It is verified against a key set per workspace rather than per instance, its claims can only ever produce a guest, and it is available on every plan.
...
For an app open to [guests](../../apps/13_guest_apps/index.mdx), the same URL carries a guest JWT in a `guest.<jwt>` last segment instead, and the token is then resolved as a guest confined to that app.

<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		title="Guest apps"
		description="Let people without a Windmill account open one app, through a sign-in or a JWT your backend signs."
