...
The Guests tab, next to Users, lists the [guests](../../core_concepts/16_roles_and_permissions/index.mdx#guests) of the trailing 30 days: people who opened an app set to Guests without a Windmill account, whether they signed in through the identity provider or came in on a [guest JWT](../../apps/13_guest_apps/index.mdx#guest-jwt-for-embedded-apps).
For each one you see their email, the workspaces they opened and when they were first and last seen.
One email is one guest, however many workspaces they opened.

Above the list, 'Allow guests on this instance' is the superadmin switch every workspace switch sits under.
...
The tab also reports the instance's standing against the guest allowance: the first 100 distinct guest emails over 30 days are free, and past that an instance holding an [Enterprise Edition](/pricing) license meters them at four guests to one seat while an instance without one admits no new guest email until the count drops back under 100.
...
### HTTP route default allowed origins

Comma-separated list of origins that [HTTP routes](../../triggers/2_http_routing/index.mdx#allowed-origins) allow to call them from a browser when the route sets none of its own, stored under `http_route_default_allowed_origins` (the API also accepts a JSON array).
A route overrides it with its own list and opts out with `*`, so this is a default rather than a ceiling.
Leave it empty for no default, so every route allows any origin unless it restricts itself.
...
### Maximum token expiration (days)

Furthest ahead, in days, that a user-created API token can expire.
A token requested with no expiration or a longer one is created with this expiration instead.
[Service account](../../core_concepts/16_roles_and_permissions/index.mdx#service-accounts) tokens are exempt.
Leave empty for no limit.
See [Maximum token expiration](../../core_concepts/59_user_tokens/index.mdx#maximum-token-expiration) for what it covers.

### Disable token in MCP URLs

When enabled, the [MCP](../../core_concepts/51_mcp/index.mdx) endpoints refuse a token passed as a `?token=` query parameter, which would otherwise end up in browser history and proxy logs.
MCP clients then sign in through [OAuth](../../core_concepts/51_mcp/index.mdx#connect-with-oauth) or send the token in an `Authorization` header.
It is off by default, and existing MCP URLs that carry a token stop working once it is enabled.
...
A [worker group](../../core_concepts/9_worker_groups/index.mdx) can cache its dependencies in its own bucket instead of this one, see [dependency cache object storage](../../core_concepts/9_worker_groups/index.mdx#dependency-cache-object-storage).
Everything else, including logs and job results, always uses the instance object storage.
...
### Auto-build binaries on deployment

When enabled and [instance object storage](#instance-object-storage) is configured, deploying a Rust, Go or C# script queues a job that compiles it and uploads the binary to the bucket, so its first run does not pay the compile. Off by default. The companion "Auto-build worker tag" setting picks the [worker tag](../../core_concepts/9_worker_groups/index.mdx) those build jobs run on. See [auto-build binaries on deployment](../../core_concepts/38_object_storage_in_windmill/index.mdx#auto-build-binaries-on-deployment).
...
<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		...
		href="/docs/advanced/setup_oauth"
	/>
</div>
...
Name of the SAML attribute or OIDC userinfo claim carrying the user's identity provider groups.
When set, every SSO login reconciles the user's membership in SCIM-provisioned instance groups against the claim before the session starts, so group changes in the identity provider apply at the next login instead of the next SCIM push.
Leave empty to disable.
Enterprise Edition only; see [login-time group sync](../28_saml_and_scim/index.mdx#login-time-group-sync-from-the-idp-groups-claim).
...
<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		...
		href="/docs/advanced/saml_and_scim"
	/>
</div>
...
By default, metadata URLs that resolve to private or internal network addresses are blocked to protect against SSRF. For self-hosted deployments with an internal IdP, set the `ALLOW_PRIVATE_SAML_METADATA_URLS=true` environment variable to allow them. See [private or internal IdP metadata URLs](../../advanced/28_saml_and_scim/index.mdx#private-or-internal-idp-metadata-urls).
...
### Mute zombie job restart alerts

Stop sending [critical alerts](../../core_concepts/37_critical_alerts/index.mdx) when a zombie job or flow is detected and restarted automatically. This setting is off by default.

A job whose worker stops pinging for longer than [`ZOMBIE_JOB_TIMEOUT`](../../core_concepts/47_environment_variables/index.mdx) is considered a zombie and restarted, up to 3 attempts, and each restart raises a critical alert. On an instance with flaky workers, that means a burst of alerts for a condition Windmill heals on its own.

Muting them leaves the recovery itself untouched, and the terminal alerts still fire: a job that exhausted all its restart attempts and is failed, and a flow cancelled after hanging between two steps.

Toggling this setting takes effect immediately, without restarting the instance.
...
<details>
  <summary>Set-up SMTP from the `.env` file (deprecated) </summary>
...
Captured spans are not kept forever: see [HTTP request tracing retention in secs](#http-request-tracing-retention-in-secs) for the retention window and its default.

### HTTP request tracing retention in secs

How long a captured [HTTP tracing](#http-tracing) span is kept, and therefore how far back the job details view can show the HTTP requests a job made.
Defaults to 7 days, and there is no "keep forever" value.

This window is independent of the job retention period, so a span can outlive its job or be deleted while the job is still listed.
Spans are usually worth expiring sooner than the jobs themselves: tracing records one per HTTP request made from a job script.

The window can also be set with the `OTEL_TRACES_RETENTION_SECS` env var on the servers, which the instance setting overrides when both are set.

:::warning Upgrading to a version that sweeps traces

Traces were never swept before this setting existed, so an instance with HTTP tracing enabled kept every span it had ever captured.
After upgrading, every span older than the retention window is deleted, with no way to recover them.

To keep more than 7 days of request traces, set the window before the upgrade rather than after: servers read `OTEL_TRACES_RETENTION_SECS` at startup, so setting it in the same deploy as the upgrade applies from the very first sweep.
:::

Capturing traces requires [Enterprise Edition](/pricing).
...
Configure a self-managed GitHub App for GitHub.com, GitHub Enterprise Cloud or GitHub Enterprise Server to enable [Git sync](../11_git_sync/index.mdx) without relying on the Windmill-managed GitHub App via `stats.windmill.dev`.

This setting is found under **Advanced > GitHub App** in Instance Settings and is only available on [Enterprise Edition](/pricing). Turn on the "Self-managed GitHub App" toggle to enable the fields below.

| Field | Description |
|-------|-------------|
| Base URL | The base URL of your GitHub instance (e.g. `https://github.com`, `https://<subdomain>.ghe.com` or your GHES URL) |
...
| App owner | The organization or user that owns the GitHub App. Only needed on GitHub Enterprise Cloud data residency hosts (`*.ghe.com`), whose installation URLs carry the owner |
| Private Key (PEM) | The private key generated for your GitHub App |

Once the app is configured, a **Workspace assignments** panel appears below the fields. Click **Refresh** to list the installations the app can see on your GitHub instance, and assign them to workspaces. This lets a superadmin link installations to workspaces directly, so workspace users do not need GitHub permissions to set up sync. It is also the recovery path for installations that were created without the app's Setup URL configured.

Installations assigned this way are tagged "Provisioned by admin" in the workspace Git sync settings and can only be removed from here, not by workspace admins.

For setup instructions, see [Self-managed GitHub App](../../integrations/git_repository.mdx#self-managed-github-app).

### Webhook base url

Base url your git host delivers [Git sync webhooks](../11_git_sync/index.mdx#webhooks) to, without trailing slash. When empty, the [base url](#base-url) is used. Set it, for the managed and self-managed GitHub Apps and for [GitLab](../../integrations/git_repository.mdx#gitlab) alike, when the git host cannot reach your instance on the url your users' browsers use.

Configure it before connecting any repository: changing it later does not move the webhooks the git host already holds, and the setting then lists the workspaces whose Git sync settings must be re-saved to move them.
