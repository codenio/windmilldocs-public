...
Base url GitHub delivers [Git sync webhooks](../11_git_sync/index.mdx#webhooks) to, without trailing slash. When empty, the [base url](#base-url) is used. Set it, for the managed and self-managed apps alike, when GitHub cannot reach your instance on the url your users' browsers use.

Configure it before connecting any GitHub App: changing it later does not move the webhooks GitHub already holds, and the setting then lists the workspaces whose Git sync settings must be re-saved to move them.
