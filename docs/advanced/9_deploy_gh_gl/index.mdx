...
- A PR is automatically created, either by Windmill itself ([in-app pull request creation](../11_git_sync/index.mdx#in-app-pull-request-creation), for repositories Windmill holds a credential for: the [GitHub App](../../integrations/git_repository.mdx#github-app) or a [GitLab project access token](../../integrations/git_repository.mdx#gitlab)) or through a CI/CD action.
...
If Windmill holds the promotion repository's credential, through the [GitHub App](../../integrations/git_repository.mdx#github-app) or a [GitLab project access token](../../integrations/git_repository.mdx#gitlab), enable the "Open a pull request for each deploy branch" toggle on the promotion repository card in the Git sync settings (it defaults on for newly configured such repositories). Windmill then opens the pull or merge request itself right after the deploy pushes its branch, and no GitHub Action is needed. See [In-app pull request creation](../11_git_sync/index.mdx#in-app-pull-request-creation).

Otherwise (a repository whose credential Windmill does not hold, or if you want custom titles or CI), set up the GitHub Action below.
