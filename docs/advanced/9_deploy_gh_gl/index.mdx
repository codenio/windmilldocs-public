...
Now you've succesfully set-up the `prod-branch` as the promotion target for the `staging` workspace. Any changes done to the staging workspace will now also be committed on a branch off of `prod-branch` named `wm_deploy/[WORKSPACE_NAME]/f/folder/item`. In the next step we will create a Github Action that automaticccaly creates a PR to merge these.
