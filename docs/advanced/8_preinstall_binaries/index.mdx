...
Init scripts are executed when the worker starts, ensuring that any necessary binaries or configurations are set up before the worker undertakes any job. They run after the worker process itself has started, however, so they are not suited to setup that must be in place before the worker makes its own network calls. CA trust is the usual example: use `RUN_UPDATE_CA_CERTIFICATE_AT_START` instead, as described in [self-signed certificates](../1_self_host/index.mdx#self-signed-certificates).
...
Any scripts are set from the [Worker Management UI](../../core_concepts/9_worker_groups/index.mdx), at the worker group level. Any script content specified in `INIT_SCRIPT` will be executed at the beginning when each worker of the worker group starts.

When adjustments are made in the [Worker Management UI](../../core_concepts/9_worker_groups/index.mdx), the workers will shut down and are expected to be restarted by their supervisor (Docker or k8s).
...
<div className="grid grid-cols-2 gap-6 mb-4">
	...
	<DocCard
		...
		description="On Enterprise Edition, worker groups can be managed through Windmill UI."
		href="/docs/core_concepts/worker_groups"
	/>
</div>
