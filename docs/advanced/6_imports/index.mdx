...
On [local development](../4_local_development/index.mdx), each script gets a content file, a metadata file and a lockfile, and lockfiles are generated and updated with [`wmill generate-metadata`](../3_cli/generate-metadata.md). See [Dependencies on local development](#dependencies-on-local-development) below.
...
To generate the lockfiles, the CLI asks the Windmill servers to run dependency jobs, using either [workspace dependencies](../../core_concepts/55_workspace_dependencies/index.mdx) (if configured) or automatically resolving from the script's imports. When a lockfile is present alongside a script at time of deployment by the CLI, no dependency job is run and the present lockfile is used instead.
