...
## Gate GitHub pull requests

When a workspace syncs to a repository through the [GitHub App](../../integrations/git_repository.mdx#github-app) with [automatic sync from Git](../11_git_sync/index.mdx#automatic-sync-from-git) enabled, pull requests from a fork or dev workspace branch get a ["Windmill CI tests" check run](../11_git_sync/index.mdx#ci-tests-check) that runs the workspace's CI tests on the pull request head.
Mark it as a required status check in GitHub to block merging until the tests pass.
This is [Enterprise Edition](/pricing) only.
