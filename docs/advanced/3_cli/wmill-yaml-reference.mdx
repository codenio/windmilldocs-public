...
| [`skipDatatableMigrations`](#skipdatatablemigrations) | `boolean` | `false` | Skip data table SQL migrations |
...
Skips the SQL [migrations](../../core_concepts/11_persistent_storage/data_tables.mdx#migrations)
of data tables, stored under `migrations/datatable/`. That root sits outside the
namespaces `includes` / `excludes` are written against, so this flag is the only
way to leave migrations out of a sync. Corresponds to the "Data table migrations"
[git sync type filter](../11_git_sync/index.mdx#type-filters).
...
Includes all [triggers](../../triggers/index.mdx) (HTTP
