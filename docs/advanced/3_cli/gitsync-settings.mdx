...
Local changes reach the workspace through one of two paths, and which one applies depends on how the repository is wired:

- `git push` - when the workspace deploys new commits itself ([automatic sync from Git](../11_git_sync/index.mdx#automatic-sync-from-git), available on [Enterprise Edition](/pricing)), or when a CI workflow runs `wmill sync push` on push.
- [`wmill sync push`](./sync.mdx#push-api) - when there is no such deploy-on-push path.

The `wmill gitsync-settings status` command reports which path applies to your workspace and current branch, so you don't have to guess:

```bash
wmill gitsync-settings status [options]
```

It reads your current local git branch and asks the backend whether pushing it is deployed by automatic sync from Git.
A `git push` is confirmed to deploy only when exactly one licensed repository with automatic sync enabled tracks the branch.
Otherwise the result is reported as not confirmed (rather than a definite no), since a CI workflow the command doesn't recognize could still deploy on push - in that case, determine the deploy path from your CI configuration or use `wmill sync push`.

The endpoint returns only derived booleans - no repository paths, credentials, or webhook configuration - so any workspace member (not just admins) can run it.
...
With `--json-output`, the command prints the deploy mode and a recommended command:
...
- `configured` - at least one git-sync repository is configured for the workspace.
- `deploy_on_push` - `true` only when pushing `current_branch` is confirmed to deploy via automatic sync from Git.
- `current_branch` - the local git branch checked (or `null` outside a git repository).
- `deploy_command` - `git push` when `deploy_on_push` is `true`, otherwise `null`.
