...
A worker with a workload identity can also use it to reach the databases your scripts and flows query, on [PostgreSQL](../../integrations/postgresql.md#azure-workload-identity-for-azure-database-for-postgresql) and [MS SQL](../../integrations/mssql.md#azure-workload-identity-for-azure-sql) resources. The sentinel there is `ms_entraid`, set as the resource password, rather than the `entraid` used in `DATABASE_URL`.
