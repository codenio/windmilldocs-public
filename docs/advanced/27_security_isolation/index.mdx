...
NSJAIL caps the virtual address space of a jailed job at `rlimit_as` MiB. For Python and Ansible jobs this defaults to 4096 MiB (4 GiB). This is a cap on virtual address _reservation_, not physical memory: JIT runtimes such as Bun (JavaScriptCore) and the JVM reserve large virtual ranges up front, so a subprocess spawned from a jailed Python or Ansible job can crash against this cap even when its actual memory use is modest.
...
Since these are set per [worker group](../../core_concepts/9_worker_groups/index.mdx), you can lift the cap only on a dedicated pool that runs these workloads. Only the address space limit is affected: the other resource limits (CPU, file size, open files) and the mount, PID and user namespace isolation that provide the actual security boundary are unchanged.
...
**`runAsUser: 0` is required.** The nsjail binary ships without file capabilities (no `setcap`), so the capabilities above are only granted automatically to a root process. Under a non-root UID they never become effective and nsjail fails with `Operation not permitted`.
...
A deployment that cannot run nsjail can rely on the process lifetime instead: with `EXIT_AFTER_N_JOBS` set, the worker exits once it has run that many jobs (`1` for one process per job) and its supervisor restarts it on a fresh process. Set it on a worker process (`MODE=worker`) running a single worker, since the exit takes the whole process down. The restart replaces the process, not the container filesystem, so caches and `/tmp` survive it. Only jobs that can leave something behind count: the worker's own [init and periodic scripts](../../core_concepts/9_worker_groups/index.mdx#worker-scripts), flow orchestration, [cache](../../core_concepts/24_caching/index.md) hits and jobs handed to [dedicated workers](../../core_concepts/25_dedicated_workers/index.mdx) do not, and a flow pinned to one worker runs to its end even past the limit.

The worker name suffix, otherwise random, is then derived from the hostname so that the restarted process keeps its entry in the workers list. Only if several worker processes of the same [worker group](../../core_concepts/9_worker_groups/index.mdx) run on one host, give each of them a distinct `WORKER_SUFFIX` to tell them apart.
...
The `sandbox` annotation can be combined with [volume annotations](../../core_concepts/67_volumes/index.mdx) to run scripts with persistent file storage inside a sandbox.
