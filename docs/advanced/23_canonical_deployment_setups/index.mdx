...
- You're deploying across multiple Windmill instances and want promotion to go through git. To deploy item by item from the UI instead, see [deploying to a workspace on another instance](../../core_concepts/12_staging_prod/index.md#deploy-to-a-workspace-on-another-instance).
...
- **Stage 4** if you need a separate production environment with a promotion gate, or you deploy across multiple Windmill instances through git (the UI can also [deploy to a workspace on another instance](../../core_concepts/12_staging_prod/index.md#deploy-to-a-workspace-on-another-instance)).
