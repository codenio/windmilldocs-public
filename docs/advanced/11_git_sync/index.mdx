...
A [dev workspace](../26_dev_workspaces/index.mdx) is the exception: it deploys to a top-level branch named after its [environment label](../26_dev_workspaces/index.mdx#environment-labels) (`dev`, `qa`, `test`, `uat`, `staging`, `demo`, `sandbox` or `preprod`) rather than to a `wm-fork/**` branch, which is why a label cannot be a branch this repository already tracks. On a repository set up for [Git Promotion](../9_deploy_gh_gl/index.mdx), a dev workspace keeps the per-item `wm_deploy/**` deploy branches instead.
