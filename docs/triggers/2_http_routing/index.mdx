...
On sync routes, a runnable can override these on its own response with [`wm_headers`](../../core_concepts/4_webhooks/index.mdx#custom-response-headers), but never on the preflight, so they cannot restrict who calls a route, and an allowlist overrides the origin they set.

### Allowed origins

To restrict which origins can call a route from a browser, turn on 'Allowed origins' in the route editor under Advanced > Request Options ('Override allowed origins' when the instance sets a default) and enter a comma-separated list, such as `https://app.example.com, https://admin.example.com`.

A listed origin gets its own value back in `Access-Control-Allow-Origin`, and any other origin gets no such header, on both the preflight and the response.
The browser then blocks the read, and a request that needs a preflight (a JSON body or an `Authorization` header, for example) never reaches the runnable.
`Access-Control-Allow-Methods` also narrows from the defaults above to the route's own method plus `OPTIONS`.
Matching is exact and ignores case, so write each origin the way a browser sends it, with no trailing slash or path.
This only restricts browsers, so use [authentication](#authentication-options) to control who can run the route.

The allowlist is resolved route first, then the [HTTP route default allowed origins](../../advanced/18_instance_settings/index.mdx#http-route-default-allowed-origins) instance setting ([Enterprise Edition](/pricing)), then any origin.
An empty list counts as unset and still follows the instance default, while `*` allows any origin even when the instance default is narrower.
[Static websites](#serving-static-files-or-websites) ignore allowlists entirely, while single static files follow them like any other route.
...
To return binary content (a PDF, image, or archive) directly from a script or flow instead of from S3, base64-encode the bytes and set `wm_content_transfer_encoding` to `"base64"` on the response. See [Binary responses](../../core_concepts/4_webhooks/index.mdx#binary-responses).
