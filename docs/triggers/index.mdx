...
- [Auto-generated UIs](./index.mdx#auto-generated-uis)
...
[Scripts](../getting_started/0_scripts_quickstart/1_typescript_quickstart/index.mdx) are sequences of instructions that automate tasks or perform specific operations. [Flows](../flows/1_flow_editor.mdx) are sequences of scripts that execute one after another or in parallel. Both are hosted in workspaces.
:::
...
![Full-code app demo](../getting_started/9_full_code_apps_quickstart/full_code_app_demo.png 'Full-code app demo')
...
These webhooks accept incoming HTTP requests, allowing users to easily trigger their Windmill scripts and flows from external services by simply sending a POST request to the appropriate authenticated webhook URL (requires passing a [token](../core_concepts/4_webhooks/index.mdx#webhook-specific-tokens) as Bearer or query arg). Their purpose is to have the script run when it receives an input from its associated webhook.
