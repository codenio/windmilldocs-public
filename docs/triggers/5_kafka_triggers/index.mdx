...
Kafka triggers support message filtering so that only messages matching specified criteria trigger the runnable. Filters match against the message parsed as JSON. The base64 encoding only applies to the payload the script or flow receives - filters run on the message before it is encoded.

Each entry is either a criterion, addressing a top-level field by Key or a nested one by a dotted Path (e.g. `account.id`) and comparing it to a value, or a group (AND, OR or NONE) nesting further entries, which lets you express a condition like "A and B and (C or D)". The filter logic selector combines the top-level entries. A human-readable preview is shown for each criterion (e.g. `payload.type == "order_created"`).

Filters are configured in the Kafka trigger editor and evaluated server-side before dispatching the job. They use the same engine as [WebSocket trigger filters](../3_websocket_triggers/index.mdx#filters), where the syntax, the matching rules and the group semantics are documented in full.
