...
<!-- SCREENSHOT PLACEHOLDER: workspace settings -> dbt, showing the warehouses table with columns Name / Resource / Target and one row named `main` pointing at a Postgres resource. File: warehouse_settings.png (+ .webp) -->

The adapter is inferred from the resource's type: `postgresql`, `redshift`, `mysql`, `duckdb`, `snowflake`, `bigquery`, `databricks`, and (on Enterprise) `ms_sql_server` and `oracledb`. Any other warehouse - ClickHouse, Salesforce, anything with a dbt adapter but no Windmill resource type - is reached through the project's own `profiles.yml` (see [Bring your own profiles.yml](#bring-your-own-profilesyml)).
...
<!-- SCREENSHOT PLACEHOLDER: a dbt run page mid-flight - the project graph with green checks on finished staging models, spinners on the marts still building, one failed model and its skipped downstream. File: dbt_run_live.png (+ .webp) -->
...
<!-- SCREENSHOT PLACEHOLDER: the folder asset graph with two dbt projects - source nodes, model nodes badged with their materialization, ref() lineage between them, and each project's script node badged with its model count. File: dbt_asset_graph.png (+ .webp) -->
