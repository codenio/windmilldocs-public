...
Windows workers enable you to run Windmill scripts and flows directly on Windows machines without requiring Docker or WSL, supporting Python, Bun, PowerShell, C#, Nu, Java, Ruby, and R executors for native Windows execution.
...
1. **Install R**: Download and install R from the [official CRAN website](https://cran.r-project.org/bin/windows/base/). During installation, ensure R is added to the system PATH. The worker invokes `Rscript.exe` through PATH on Windows.
   - Start PowerShell and verify you have a working R installation:
     ```powershell
     Rscript --version
     ```

2. **Locate the R installation**:
   - Find where `Rscript.exe` is installed by running:
     ```powershell
...
3. **Set environment variables**:
   - Add the following environment variables (replace the placeholders with your specific values):
     ...
     ```
     Without `RSCRIPT_PATH`, R jobs fail with "Rscript binary not found" even when `Rscript.exe` is on PATH.

4. **Install renv**: Windmill uses [renv](https://rstudio.github.io/renv/) to resolve and install R package dependencies, so it must be available to the worker's R installation:
   - Run:
     ```powershell
