...
The same tab has a toggle to [hide the assistant](#hiding-the-ai-assistant-in-a-workspace) from the workspace UI.
...
In its global mode, the chat footer has an assistant settings button that both accounts for what the chat carries into a turn - its tools, the AI skills and MCP servers turned on for it, the custom instructions in its system prompt and the files linked to it - and is where each of those is changed.
It replaces the earlier context popover and the separate skills and MCP drawers: see [assistant settings](../69_ai_sessions/index.mdx#assistant-settings).
...
## Hiding the AI assistant in a workspace

Workspace [admins](../16_roles_and_permissions/index.mdx#admin) can hide the Windmill AI assistant from a whole workspace with the "Hide AI sessions" toggle at the bottom of the "Windmill AI" tab of the [workspace settings](../44_workspace_settings/index.mdx).
[AI sessions](../69_ai_sessions/index.mdx), the AI chat, code generation and completion, "AI Fix" and the other AI buttons then disappear for every member.

[AI agent](../54_ai_agents/index.mdx) steps and the [AI sandbox](../68_ai_sandbox/index.mdx) keep working: the toggle only hides the assistant in the UI, it does not restrict API access to the configured providers.
