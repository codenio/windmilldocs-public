...
The AI sandbox combines two Windmill features - [sandboxing](../../advanced/27_security_isolation/index.mdx#sandbox-annotation) and [volumes](../67_volumes/index.mdx) - to run AI coding agents with process isolation and persistent file storage. Any agent that operates on a local filesystem can be wrapped in a Windmill [script](../../getting_started/0_scripts_quickstart/index.mdx) this way: [Claude Code](https://docs.anthropic.com/en/docs/claude-code), [Codex](https://github.com/openai/codex), [OpenCode](https://github.com/sst/opencode), or a custom agent.
...
- `sandbox` - runs the [job](../20_jobs/index.mdx) inside [nsjail](../../advanced/27_security_isolation/index.mdx#nsjail-sandboxing), isolating the agent's filesystem and processes from the [worker](../9_worker_groups/index.mdx).
- `volume: <name> <path>` - mounts a persistent [volume](../67_volumes/index.mdx) so the agent can read and write files that survive across job runs (session state, memory, generated artifacts, etc.). Files are synced to [object storage](../38_object_storage_in_windmill/index.mdx).
