...
2. Optionally set an **expiration date**.
   If your instance sets a [maximum token expiration](#maximum-token-expiration), the expiration can't go past it.
...
Superadmins can cap how long user tokens live with the "Maximum token expiration (days)" [instance setting](../../advanced/18_instance_settings/index.mdx#maximum-token-expiration-days) (`max_token_expiration_days`), a whole number of days.
Leave it empty for no limit.

The limit applies to tokens created from Account settings, the [CLI](../../advanced/3_cli/index.mdx) (including browser login with `wmill workspace add`) and the API.
A token requested with no expiration, or with one past the limit, is created with the limit as its expiration instead of being refused.
The limit is checked when a token is created, so setting or lowering it does not change existing tokens.

The limit does not apply to:

- Tokens of [service accounts](../16_roles_and_permissions/index.mdx#service-accounts), so automation can keep long-lived credentials.
- Tokens Windmill creates on its own, such as login sessions, [native trigger](../../triggers/11_native_triggers/index.mdx) webhook tokens and app embed tokens.
