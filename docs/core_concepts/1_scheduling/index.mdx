...
A particular use case for schedules are [Trigger scripts](../../flows/10_flow_trigger.mdx): schedule a script that pulls data from an external source and have it return all of the new items since the last run, as scheduled polls, without resorting to external webhooks.
...
Schedule error handler is an [Enterprise Edition](/pricing) feature.

### Schedule recovery handler

From the schedule configuration, add a special script or flow to execute in case of recovery from error.
