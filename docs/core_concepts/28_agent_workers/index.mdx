...
In terms of [billing](../../enterprise/1_plans_details/index.mdx), agent workers are counted as 1 [Compute Unit](/pricing) per agent worker on self-hosted EE (if not used for compute) and 0.5 Compute Unit on cloud EE (if not used for compute).
