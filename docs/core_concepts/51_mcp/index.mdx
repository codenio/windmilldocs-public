...
The two write tools deploy [full-code apps](../../full_code_apps/index.mdx): they take the app's source files and [compile them on a worker](../../full_code_apps/7_deployment/index.mdx#deploy-from-sources-via-the-api), returning compile errors as the error of the call so the agent can fix its own code. Granting either lets the agent make a worker build the sources it wrote, so the token needs the `jobs:run` scope as well as `apps:write`.
