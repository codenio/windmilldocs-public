...
to them. They are at the basis of [Windmill's architecture](../../advanced/1_self_host/index.mdx#architecture) as they run the jobs.
...
- `dbt`: The default worker group for [dbt](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx) projects.
...
It is possible to restrict some tags to specific workspaces using the following syntax:

```
gpu(workspace+workspace2)
```

Only `workspace` and `workspace2` will be able to use the `gpu` tag. The exclusion form does the opposite: `gpu(^workspace)` lets every workspace except `workspace` use the tag.

By default a scope entry matches only the workspace it names, so a [workspace fork](../../advanced/20_workspace_forks/index.mdx) does not inherit its parent's scoped tag. Append `*` to a workspace id to extend that entry to the workspace's forks, transitively:
...
sensitive(^prod)                  everyone except prod itself (forks still allowed)
```

The marker is opt-in per entry, so existing tag strings keep their exact meaning: a fork inherits a scoped tag only where you explicitly opt in with `*`. This is useful when the tag names a shared dev worker group; leave it off when a worker group is restricted precisely because a fork's code has not been reviewed yet. Assignable tags are configured in the instance settings, which requires superadmin privileges.
...
If a workspace tag contains the substring `$workspace`, it will be replaced by the workspace id the job routes to. This is especially useful to have the same script deployed to different workspace and have them run on different workers.

That id is the workspace's own for a standalone workspace and for a [dev workspace](../../advanced/26_dev_workspaces/index.mdx), which is a workspace an admin names and provisions workers for. For a [fork](../../advanced/20_workspace_forks/index.mdx) it is the nearest such ancestor instead: a fork's id is generated per fork, so interpolating it would produce a tag no worker listens to and the job would stay queued.
...
the workspaces, `dev`, `staging`, `prod` and the worker groups: `normal-dev`, `normal-staging`, `normal-prod`. The same script with the tag `normal-$workspace` will run on the corresponding worker group depending on the workspace it is deployed to. This enable to share the same control plane but use workers with different network restrictions for tighter security.
...
On a flow step, the tag can also contain `$flow_expr[path]`, which reads from the flow's state instead of the step's inputs, so a step can run on a worker group chosen by an earlier step's result, a flow input or a flow env variable.
The path starts with one of:

- `results.<step_id>`: the result of a previous step, including a step of the enclosing flow from inside a [loop](../../flows/12_flow_loops.md) or [branch](../../flows/13_flow_branches.md).
- `flow_input.<key>`: a [flow input](../../flows/3_editor_components.mdx#flow-inputs). Inside a for loop, `flow_input.iter.value` is the current item.
- `flow_env.<key>`: a [flow env variable](../../flows/3_editor_components.mdx#flow-env-variables).

For example, with the tag `gpu-$flow_expr[results.a.region]`, if step `a` returned `{"region": "eu"}`, the step runs on the `gpu-eu` worker group.
A path that reaches nothing resolves to an empty string, and a tag left empty runs on the default tag.
When you test the step on its own, there is no flow state to read, so it also runs on its default tag.

To make such a tag assignable, add the template itself (e.g. `gpu-$flow_expr[results.a.region]`) to the assignable tags.
Any value it resolves to is then allowed.
...
The same resolution applies to workspace-specific default tags: when they are enabled, [workspace forks](../../advanced/20_workspace_forks/index.mdx#workspace-specific-tags-and-forks) resolve to their nearest provisioned ancestor, so fork jobs run on that workspace's workers. An optional `-fork` suffix can be enabled in instance settings to route fork jobs to dedicated workers instead.

[Concurrency keys](../21_concurrency_limits/index.md), [volume names](../67_volumes/index.mdx) and dedicated workers are not affected: `$workspace` there is still the workspace's own id, so a fork keeps its own concurrency budget and a dedicated worker keeps running the code version it was pinned to.
...
## Dependency cache object storage

The [distributed dependency cache](../38_object_storage_in_windmill/index.mdx#instance-object-storage-distributed-cache-for-python-rust-go) normally lives in the [instance object storage](../38_object_storage_in_windmill/index.mdx#instance-object-storage).
A worker group can point it at its own bucket instead, from the "Dependency cache object storage" section of its config.
This is meant for workers that sit far from the instance bucket, or cannot reach it at all: a regional pool that would otherwise pull every virtual env, bundle and compiled binary across a WAN on each cold start.

This is an [Enterprise Edition](/pricing) feature, and takes the same S3, Azure Blob, Google Cloud Storage or AWS OIDC settings as the instance object storage.

Only the dependency cache moves to the group's bucket: Python virtual envs, language dependency bundles, and compiled Go, Rust, C# and Java binaries.
Everything else keeps using the instance object storage, including job results, [service and job logs](../36_service_logs/index.mdx), [codebases](../33_codebases_and_bundles/index.mdx) uploaded at deploy time, app assets and [AI agent](../54_ai_agents/index.mdx) conversation memory.

The override applies without restarting the workers.
Everything it holds can be rebuilt from a lockfile, so while the bucket is misconfigured or unreachable, the group's cache stays local to each worker instead of falling back to the instance bucket: cold starts get slower, jobs keep running.

Bucket credentials are masked for [devops](../16_roles_and_permissions/index.mdx#devops) users who are not instance admins, and saving the group from that view keeps the stored credentials.
The config key is `object_store_cache_config`, so the override can also be set through [config as code](../57_infrastructure_as_code/index.mdx#worker-group-config-fields).
[Agent workers](../28_agent_workers/index.mdx) read their tags from their token rather than from a group config, so the override does not apply to them.

<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		title="Instance object storage"
		description="Connect your instance to a bucket for large logs and the distributed dependency cache."
