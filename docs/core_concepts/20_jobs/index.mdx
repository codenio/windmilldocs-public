...
See [Streaming](../58_streaming/index.mdx) for all streaming options, including streaming a flow's progress step by step.
...
The HTTP request traces captured by [HTTP tracing](../../advanced/18_instance_settings/index.mdx#http-tracing) are not covered by this retention period.
They have their own window, [configured separately](../../advanced/18_instance_settings/index.mdx#http-request-tracing-retention-in-secs), so a trace can outlive its job or be deleted while the job is still listed.
...
For flows, updates carry the `flow_status` with the status of every step whenever it changes, which lets you show a flow's progress step by step.
See [Stream flow progress](../58_streaming/index.mdx#stream-flow-progress) for a complete example.

Windmill provides an [SSE endpoint](https://developer.mozilla.org/en-US/docs/Web/API/Server-sent_events/Using_server-sent_events) to watch a given job for new logs or completion. The API endpoint is [/api/w/\{workspace\}/jobs_u/getupdate_sse/\{id\}](https://app.windmill.dev/openapi.html#tag/job/get/w/{workspace}/jobs_u/getupdate_sse/{id})
...
fast (optional boolean, default false): check for updates every 100ms, then every 500ms, before settling at every 3s. Without it, updates are checked every 3s
poll_delay_ms (optional integer, Enterprise Edition): fixed interval in ms between update checks, minimum 50
```
...
- **Update**
  ...
  The updates will only contain new updates since the last one, no update will be sent if there are no updates to share, otherwise updates are sent at most as often as the server checks for them (see `fast` and `poll_delay_ms` above).
