...
Workspace object storage is also used as the backend for [volumes](../67_volumes/index.mdx), which provide persistent file storage for scripts.
...
The SDK functions that take an `S3Object` also accept an `s3://` URI string. The format is strict: `s3://<storage>/<key>`, where `<storage>` is the name of a [secondary storage](#secondary-storage) and is left empty for the default workspace storage. A file `path/to/file` in the default storage is therefore written `s3:///path/to/file`, with three slashes.
...
The same `s3:///<key>` spelling is used by [asset](../52_assets/index.mdx) annotations and DuckDB queries (`read_json_auto('s3:///path/to/file')`), so one spelling works everywhere and resolves to the same asset path. This is particularly relevant when connecting scripts into [pipelines](../63_pipelines/index.mdx).
...
This can be configured from the [instance settings](../../advanced/18_instance_settings/index.mdx#instance-object-storage), with configuration options for S3, Azure Blob, Google Cloud Storage, or AWS OIDC. For each you will find a button to test settings from a server or from a worker.
...
For large logs storage (and display) and cache for distributed Python jobs, you can [connect your instance to a bucket](../../advanced/18_instance_settings/index.mdx#instance-object-storage). This feature is at the Instance-level, and has no overlap with the Workspace object storage.
...
The binary of a [Rust](../../getting_started/0_scripts_quickstart/9_rust_quickstart/index.mdx), [Go](../../getting_started/0_scripts_quickstart/3_go_quickstart/index.mdx) or [C#](../../getting_started/0_scripts_quickstart/11_csharp_quickstart/index.mdx) script is compiled lazily, by the first worker that runs it. With instance object storage configured, "Auto-build binaries on deployment" (`auto_build_binary_on_deploy`, env fallback `AUTO_BUILD_BINARY_ON_DEPLOY`) in the [instance settings](../../advanced/18_instance_settings/index.mdx#auto-build-binaries-on-deployment) makes each deployment of such a script queue a job that compiles it and uploads the binary to the bucket instead, so no run ever pays the compile. It is off by default and does nothing without instance object storage. A script that does not compile fails on that build job, at deployment time, rather than on its first run - the deployed script itself is left intact. All [workers](../9_worker_groups/index.mdx) must be on version 1.789.0 or later.

"Auto-build worker tag" (`auto_build_binary_tag`, env fallback `AUTO_BUILD_BINARY_TAG`) pins those builds to a [worker tag](../9_worker_groups/index.mdx); left empty, they run on the script's language tag. Point it at a group that has the toolchains and matches the platform of your runtime workers, since the cache key includes the OS and architecture. That group then runs the script author's build steps (Cargo build scripts, MSBuild targets, cgo) at deployment time, with the same isolation a first-run compile has.
...
#### Per worker group cache bucket

Under [Enterprise Edition](/pricing), a [worker group](../9_worker_groups/index.mdx#dependency-cache-object-storage) can cache its dependencies in its own bucket instead of the instance one, from the "Dependency cache object storage" section of its config.
This is for workers that sit far from the instance bucket, or cannot reach it at all, and only covers what a worker rebuilds by itself: virtual envs, language bundles and compiled binaries.
Job results, logs, codebases and app assets keep using the instance object storage on every group.

<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		title="Dependency cache object storage"
		description="Point a worker group's dependency cache at its own bucket."
		href="/docs/core_concepts/worker_groups#dependency-cache-object-storage"
	/>
</div>
