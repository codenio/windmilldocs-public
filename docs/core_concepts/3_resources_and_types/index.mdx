...
For resource types backed by an OAuth provider, the token is fetched from the provider and refreshed automatically instead of being entered by hand. The dialog lists providers configured on the instance under "Instance-configured OAuth APIs", and every other resource type under "Others". Picking a provider from "Others" lets you connect it with your own [client credentials](../../advanced/27_setup_oauth/index.mdx#client-credentials) rather than the instance-wide ones. See [Setup OAuth and SSO](../../advanced/27_setup_oauth/index.mdx) to configure providers.
...
Every change to a resource's value is kept as a version. Recording happens at the database level rather than in the editor, so it covers every write: edits from the UI, pushes from the [CLI](../../advanced/3_cli/index.mdx), `setResource` calls from a script, [workspace forks](../../advanced/20_workspace_forks/index.mdx) cloning resources, and [trashbin](../61_trashbin/index.mdx) restores.

Open the history with the "History" button in the resource editor. Versions are listed newest first with their author and date, and can be viewed or diffed against the current value.

{/* Placeholder: screenshot of the resource editor's History drawer - version list on the left with the top entry labelled "Current", and the right pane on the "Diff with current" view showing a JSON diff. */}

"Restore this version" writes the selected value back onto the resource, and requires write access to it. It is appended as a new version rather than rewinding, so the history stays append-only and a restore can itself be undone. If the version references a `$var:` or `$res:` path that no longer exists, the drawer warns you first: the restore still goes through, but the resource points at something unresolvable until you recreate the reference.

Windmill keeps up to 100 versions per resource and trims older ones in the background. Renaming a resource carries its history to the new path; deleting a resource deletes its history with it, so a trashbin restore starts a fresh history.

[States](#states) and [caches](../24_caching/index.md) are excluded: resources of type `state` and `cache` are rewritten by every job that uses them, so versioning them would record machine churn rather than intent.

:::caution Past values remain readable

Overwriting a resource value no longer erases it - anyone who can read the resource can read its past versions. A secret stored as a [variable](../2_variables_and_secrets/index.mdx) reference (`$var:`) is safe: only the reference is kept in the resource value and it is resolved at fetch time, so the secret itself never enters the history. A value that inlines a credential instead - typed straight into a field, pushed by the CLI, or written by `setResource` - keeps that credential readable in every past version that held it. After rotating such a credential, use "Clear past versions" in the History drawer, which deletes every version but the current value for that resource. It is only offered to [owners](../16_roles_and_permissions/index.mdx) of the resource path. On [cloud](../../enterprise/1_plans_details/index.mdx), workspace admins can also prune the version history of every resource at once from the workspace quotas page.

:::

The same operations are available from the API: [`getResourceHistory`](https://app.windmill.dev/openapi.html#/operations/getResourceHistory) lists a path's versions, [`getResourceVersion`](https://app.windmill.dev/openapi.html#/operations/getResourceVersion) returns one version with its value, [`restoreResourceVersion`](https://app.windmill.dev/openapi.html#/operations/restoreResourceVersion) restores it, and [`clearResourceHistory`](https://app.windmill.dev/openapi.html#/operations/clearResourceHistory) drops every version but the current value.

<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		title="Versioning"
		description="Scripts, flows and apps keep a version for every deployment."
...
A resource type can also travel with a [Hub project](../../misc/1_share_on_hub/index.md#resource-dependencies) when you publish a folder. There, exporting a type's definition is an explicit per-type opt-in, meant for custom types the Hub does not already define.
...
From the code editor's toolbar, click on the `+ Type` button and pick the right resource type. For example, to access the `u/user/my_postgresql` resource of the `postgresql` Resource Type we would create a script:
...
Everything the manager does runs as a [job](../20_jobs/index.mdx) tagged with the database's language (`postgresql`, `mysql`, `duckdb`, etc.), so the [tag](../9_worker_groups/index.mdx#set-tags-to-assign-specific-queues) decides which [worker group](../9_worker_groups/index.mdx) picks it up.

The "Worker tag" button in the action bar runs them on a custom tag instead. Use it when the database is only reachable from one worker group (a private network or VPC), or when no worker listens to the language's tag. The tag is remembered per workspace and database in your browser, and also applies to the "Data" drawer of [full-code apps](../../full_code_apps/index.mdx).
