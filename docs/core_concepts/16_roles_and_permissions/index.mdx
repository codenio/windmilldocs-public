...
Guests are individuals with no Windmill account, members of no workspace, whom your [identity provider](../../advanced/27_setup_oauth/index.mdx) authenticates or an embedding backend names in a [signed JWT](../../apps/13_guest_apps/index.mdx#guest-jwt-for-embedded-apps).
...
Entering as a guest creates no user and no workspace membership, so guests do not take a seat.
