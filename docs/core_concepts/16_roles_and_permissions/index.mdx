...
[Guests](#guests) have no account at all and are not counted as users.
They count against a separate [guest allowance](../../apps/13_guest_apps/index.mdx#seats-and-the-guest-allowance) instead.
...
With [login-time group sync](../../advanced/28_saml_and_scim/index.mdx#login-time-group-sync-from-the-idp-groups-claim) enabled (Enterprise Edition), membership in SCIM-provisioned groups is also re-evaluated from the identity provider's groups claim at each SSO login, so a role granted or revoked through a group applies at the user's next login instead of waiting for the next SCIM push.
...
Operators have limited access within a workspace. They can execute and view scripts, flows, and apps that they have visibility on, and only those that are within their allowed path or [folders](../8_groups_and_folders/index.mdx).

What the Operator role restricts is **authoring and running code**. An operator cannot:

- create, edit, archive or delete [scripts](../../getting_started/0_scripts_quickstart/index.mdx), [flows](../../getting_started/6_flows_quickstart/index.mdx) and [apps](../../getting_started/7_apps_quickstart/index.mdx);
- run previews (any code that is not an already-deployed script or flow) or dependency jobs;
- [deploy to another workspace](../12_staging_prod/index.md): the deploy UI is closed to operators, and creating the script, flow or app on the target side is refused anyway.

Everything else follows the regular [permission model](#permissions-and-access-control). The Operator role is not, by itself, read-only. Within the paths and [folders](../8_groups_and_folders/index.mdx) they have been granted write access to, an operator can create and edit [variables](../2_variables_and_secrets/index.mdx), [resources](../3_resources_and_types/index.mdx), [schedules](../1_scheduling/index.mdx), [triggers](../../triggers/index.mdx), folders and [groups](../8_groups_and_folders/index.mdx#groups), exactly like any other member. If you need an operator to be strictly read-only, grant them read-only access to the folders and paths they need, and do not rely on the role for it.
...
The second option is to share the script and the [variables](../2_variables_and_secrets/index.mdx) it depends on. An operator with read access to a variable can read its value, secrets included, through the API. The UI hides the "Load secret value" button for operators, but that is a UI convenience, not a permission boundary. Grant folder access accordingly.

From the workspace settings, you can configure the operator visibility settings for your workspace. These control which pages an operator sees; they do not change what an operator is allowed to write, and they are all enabled by default. You can allow/disallow operators to view:
...
Service accounts are workspace-scoped automation identities that cannot log in directly (no password, no SSO). They act through [tokens](../59_user_tokens/index.mdx) and are intended for CI/CD pipelines, scheduled jobs, or API integrations that need workspace-scoped permissions without a real user login.

Service accounts are an [Enterprise](/pricing) feature. Create them from workspace settings under user management: in "Add new user", pick **Service Account** as the role. Only a username is required — the email is derived automatically, so there is nothing to invite or verify.

A service account is **not** necessarily an [operator](#operator). You pick its role at creation time:

- **Operator** (default): read/run only. Cannot be used for CLI sync or to author scripts, flows and apps.
- **Developer**: can author and edit scripts, flows and apps within its path. This is the role to use for CLI sync tokens.
- **Admin**: full workspace admin. Grant only when the service account needs to manage workspace settings.

When you pick Developer, you can also add the service account to the **`wm_deployers`** group. This is recommended when the account is used as a `wmill sync push` / CI deploy identity, since members of `wm_deployers` can [deploy on behalf of](../12_staging_prod/index.md#run-on-behalf-of) other users in the target workspace.

Admins can **impersonate** a service account to act as that identity and verify its permissions.

Service accounts count toward [seats](#users) with the same weighting as regular members: an operator service account counts as half a seat, a developer or admin service account as a full seat.
...
Guests are individuals your [identity provider](../../advanced/27_setup_oauth/index.mdx) authenticates who have no Windmill account and are members of no workspace.
A guest can open an app deployed in guest execution mode and nothing else: no listing of jobs, scripts, flows, apps or variables, no other app, no other workspace.
Like for an anonymous viewer, the app's runnables execute on behalf of the app publisher, within the app's automatically generated policy.

Signing in as a guest creates no user and no workspace membership, so guests do not take a seat.
They count instead against an instance-wide allowance: the first 100 distinct guest emails over a trailing 30 days are free everywhere, and past that they are either metered or refused depending on the instance's license.
See [seats and the guest allowance](../../apps/13_guest_apps/index.mdx#seats-and-the-guest-allowance).

Someone who already has an account anywhere on the instance is never admitted as a guest, and a guest who later signs in from the ordinary login page becomes a regular user, counted like any other.
...
<div className="grid grid-cols-2 gap-6 mb-4">
	...
	<DocCard
		color="orange"
		title="App access control (App editor)"
		description="Open a deployed app to members, to guests, or to anyone who knows the secret URL."
...
For scripts, flows, apps and variables, those extra permissions are also tracked in source as an `extra_perms` map, so they can be reviewed and deployed like the rest of the workspace through [Git sync](../../advanced/11_git_sync/index.mdx#granular-permissions) or the [CLI](../../advanced/3_cli/sync.mdx#granular-permissions-extra_perms).
...
The setting itself lives in the script or flow's advanced settings ("Run on behalf of"), but you don't need to open the editor to see it: the detail page header of a runnable that has it set shows an `On behalf of u/<user>` badge naming the identity every run is permissioned as, whoever starts it. [Operators](#operator), who have no access to the editor, see the badge too.
