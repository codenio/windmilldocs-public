...
- [dbt models](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx#models-in-the-asset-graph) : dbt://warehouse/schema/model
...
Volume assets are the exception: a `volume://` asset comes from a [`volume:` annotation](../67_volumes/index.mdx#declaring-volumes) rather than from an analysis of the code, so it is detected in every language that accepts the annotation, PHP and Go included.
...
For [dbt](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx) relations, a project that opts into dbt's static-analysis pass publishes the real column schema of each model and its [column-to-column lineage](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx#column-level-lineage), which the details pane draws as a trace above the model's SQL.
