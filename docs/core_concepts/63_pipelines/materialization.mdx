---
description: How do I materialize tables in Windmill pipelines? Managed DuckLake writes, SCD2 history, schema capture, data tests and declared warehouse relations.
---
...
// materialize manual dbt://<warehouse>/<schema>/<name>                                # track-only warehouse relation, any language but dbt's own
```
...
- **`dbt://<warehouse>/<schema>/<name>`** is a warehouse relation rather than a lake table: track-only by construction (`manual` is the only mode) and open to any language but dbt's own. See [Declaring a warehouse relation write](#declaring-a-warehouse-relation-write).

A `ducklake://` target is for DuckDB SQL, managed and `manual` alike: the write engine is DuckDB's, so the annotation is rejected at deploy on any other language. From **Python or TypeScript**, use the `wmill.ducklake` helpers (`upsert_partition`, `append_partition`, `read`) which give the same managed, idempotent write from arbitrary code.
...
## Declaring a warehouse relation write

The other target kind is a warehouse relation, the same `dbt://<warehouse>/<schema>/<name>` node a [dbt project](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx#models-in-the-asset-graph) puts on the graph.
A script in any language but dbt's own can declare that it writes one:
...
def main():
    # Your own INSERT / COPY / MERGE against the warehouse, in whatever
    # client the language uses. Windmill records the outcome, not the write.
    ...
```

Identity is the physical relation, never the tool that produced it, so the ingestion script above and a dbt model reading `analytics.raw_orders` as a `source` land on the same node.
That is what makes an ingestion step and the dbt project consuming it one lineage instead of two disconnected pictures, and it is what lets a downstream script subscribe with `// on dbt://main/analytics/raw_orders`.

`manual` is not one mode among several here, it is the only one: Windmill generates no warehouse DDL, so the script issues its own write and Windmill records what happened.
Writing the annotation without `manual` is refused at deploy rather than quietly downgraded, so nobody ships it expecting a generated write.

Checked at deploy:

- The target is a whole three-segment relation. `dbt://main/analytics` is refused, since no producer and no reader can ever name it.
- `<warehouse>` is one the workspace [configures](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx#configure-a-warehouse), resolved the same way a dbt project's `profile.warehouse` is. A name no warehouse answers to would strand the write on a node nothing else reaches.
- A dbt script itself may not declare it: what a project builds is read from its `manifest.json` and republished on every deploy, so an annotated write would be wiped by the deploy that accepted it.
- [`// data_test`](#data-tests) beside a `dbt://` target is refused. Those checks are probes spliced around a *managed* write, and a warehouse relation is written by the script itself, so nothing would run them. Assert on the relation with a dbt test in the project that reads it.

What a run records is the same track-only edge a `manual` DuckLake target gets: the relation is marked materialized (or failed, with the error) and stamped with the producing job, keyed by the [partition](./partitioning.mdx) when the script is also `// partitioned`.
Everything the managed path adds is absent: no snapshot id, no row count, no [schema capture](#schema-capture) and so no [contract warnings](#schema-contracts) for consumers.
The [partition-status grid and backfill](#partition-status-and-backfill) are DuckLake-only surfaces, so unlike a `manual` DuckLake target a warehouse relation gets neither.
The [cascade](./index.mdx#triggering-a-pipeline) is unaffected: it fans out from the deploy-time asset rows, so subscribers on the relation fire when the script completes.
A `// on dbt://<relation>` subscription goes through the same relation and warehouse checks as the target above, and is refused when every script that writes the relation is a dbt one, since a dbt run does not dispatch.
Nothing producing the relation yet is fine: a subscriber may be deployed before its producer, as for every other asset kind.

Like every other runtime pipeline annotation, the recording happens on the normal execution path.
A run handed to a [dedicated worker](../25_dedicated_workers/index.mdx) or a flow runner skips that path, exactly as it skips `// partitioned` resolution, so such a run performs its write and leaves the relation with no last writer.
...
A `// column` source can name a [dbt](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx) model's column as well as a `ducklake://` one:

```sql
-- column amount <- dbt://main/analytics/orders.amount
```

That names the same column node dbt's own [column-level lineage](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx#column-level-lineage) does, so the pipeline's graph and the dbt project's are drawn as one.
Selecting a DuckLake table traces back through the script that wrote it into the dbt models that fed it, and selecting a dbt model traces forward into the pipeline that derives from it.
The dbt half only exists for a project that opted into dbt's static-analysis pass, and it is fetched per selection, so it lands a moment after the selection does.
