...
// materialize ducklake://<name>/<table> … on_schema_change=ignore                     # any managed form: mute downstream contract warnings
...
- **Managed (default)** — Windmill generates the write. DuckDB-only; validated at deploy (a script that is not a single trailing SELECT is rejected with a clear error). The **strategy** decides how each slice is reconciled:
  ...
  - `on_schema_change=ignore` combines with any strategy: it declares the table's schema deliberately unstable and suppresses downstream [schema contract](#schema-contracts) warnings. Default is `warn`.
...
### Suppressing warnings for an unstable schema

When a table's schema is deliberately unstable - a raw landing table whose columns track an external payload, for example - declare it on the producer with `on_schema_change=ignore`:
...
Consumers of that asset then get a single informational note instead of per-column warnings. The default is `warn`, and unknown values keep the default. On `// materialize manual` the option is inert since nothing is captured; the deploy logs a warning saying so.
