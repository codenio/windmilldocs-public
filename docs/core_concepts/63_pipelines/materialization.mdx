...
| `fail` | The run aborts when the SELECT's column set diverges from the stored schema. It compares the set of column *names*, so it catches added, dropped or renamed columns, but not a same-name column reordering (which keeps the same set). |
...
- **Read the freshly-materialized target through the internal `_wm_target` schema.** Windmill attaches the slice this run just wrote as `_wm_target`, so reference the target as `_wm_target.<table>` (the table name is the last segment of the `ducklake://` URI). Reading the target this way sees this run's slice, not stale committed data - on Enterprise Edition it is the pre-commit write being audited (see [write-audit-publish](#failing-tests-publish-or-roll-back-ce-vs-ee)). You do not `ATTACH` the target yourself.
- **Exactly one trailing `SELECT`, returning the offending rows.** Setup statements (`ATTACH` of *other* lakes, `SET`, `CREATE TEMP …`) may precede it, so you can join the target against another table for a cross-table check; the last statement must be the violating-rows SELECT.
