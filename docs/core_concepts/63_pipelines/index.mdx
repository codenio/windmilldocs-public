...
Managed DuckLake writes - the `// materialize` grammar and strategies, SCD2 history, versioning and time-travel, partition status and backfill, schema capture, column-level lineage and data tests - are covered on the dedicated [Materialization](./materialization.mdx) page.
