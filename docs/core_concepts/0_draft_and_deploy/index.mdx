...
[AI sessions](../69_ai_sessions/index.mdx) can create and edit drafts for workspace items including scripts, flows, apps, raw apps, resources, variables, and triggers. When an AI session modifies an item's draft, you will see the changes appear live in your editor.
...
When you are happy with the changes, ask the chat to review them: it opens the [Compare & Deploy](../../advanced/20_workspace_forks/index.mdx#merge-workspaces-from-the-ui-merge-ui) page with the session's edited items preselected, ready to deploy.
