...
On the Community Edition, total workspace storage is limited to 10GiB per workspace (there is no per-file size limit) and the bucket browser will not work for buckets containing more than 20 files. Consider upgrading to Windmill [Enterprise Edition](/pricing) for unlimited workspace storage.
...
We recommend using the native DuckDB scripts integration, which uses the Windmill [S3 proxy](../38_object_storage_in_windmill/index.mdx#s3-proxy) with no additional configuration needed.
