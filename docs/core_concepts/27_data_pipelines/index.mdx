...
An existing [dbt](../../getting_started/0_scripts_quickstart/16_dbt_quickstart/index.mdx) project runs on Windmill as-is: one project is one script, dbt still owns its models, refs and tests, and those models land on the same asset graph as the steps above.
...
These figures are backed by our [TPC-H-derived benchmark](https://github.com/windmill-labs/windmill-benchmarks/tree/main/pipelines) (refreshed 2026): DuckDB, querying Parquet directly, ran a 9-query workload over a 100 GB dataset (600M-row fact table) in ~20 seconds at a ~20 GB memory peak on a single node — beating a single-node Spark baseline at every scale.
